# Nó Tático - Teste de simulador de partidas - 15/09/2022
# (Versão baseada no simulador da Copa do Mundo)
...
# ____________________________ Jogo Normal __________________
    for minutagem in range(1, tempo_jogo):
        print('{} minutos de jogo'.format(minutagem))
        sleep(0.3)
        ...
        if (meioA > (meioB - coletivoA/2) and ataqueA > defesaB
                and chanceA == 1):
            golsA += 1
            print("GOOOOOL DO Palmeiras!! Marcado por {}".format(jogA[c_g_jog]))
            texto_GA.append(f"{jogA[c_g_jog]}, {minutagem}'")
        if (meioB > (meioA - coletivoB/2) and ataqueB > defesaA
                and chanceB == 1):
            golsB += 1
            print("GOOOOOL DA Seleção do Mundo!! Marcado por {}".format(jogB[c_g_jog]))
...
if __name__ == '__main__':
    jogaA = ['Weverton',
             'Piquerez', 'Luan', 'Gómez', 'Mayke',
             'Zé Rafael', 'Danilo', 'Raphael Veiga', 'Dudu',
             'Scarpa', 'Rony']
    jogaB = ['Buffon',
             'Marcelo', 'Sergio Ramos', 'Beckenbauer', 'Cafú',
             'Pelé', 'Iniesta', 'Messi', 'Cristiano',
             'Maradona', 'Garrincha']

    val_jogo = resultado(59, 99, jogaA, jogaB)
    print(f"""
          Palmeiras {val_jogo[0]} x {val_jogo[1]} Seleção do Mundo
          """)
